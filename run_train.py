...
def run_one(model_name, device):
    ...
    if model_name in {"yolov8n", "yolov10n"}:
        ...
        trainer = YOLOTrainer(
            DATA_YAML, ckpt=ckpt, name=model_name, hp=HP, device=device
        )
    elif model_name == "frcnn":
        trainer = FasterRCNNTrainer(
            IMAGES, LABELS, num_classes=10, hp=HP, device=device
        )
    elif model_name == "ssd":
        trainer = SSDTrainer(IMAGES, LABELS, num_classes=10, hp=HP, device=device)
    else:
        trainer = ResNetClassifier(
            IMAGES, LABELS, hp=dict(epochs=50, batch=16, lr=1e-4), device=device
        )
...
def main():
    ...
    ap.add_argument(
        "--model",
        choices=["yolov8n", "yolov10n", "frcnn", "ssd", "resnet50", "all"],
        required=True,
    )
